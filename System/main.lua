...
function LaunchProgram(path, args, title) -- функция запуска программки
	...
	-- НАКОНЕЦТО СДЕЛАЛ ГЛОБАЛЬНЫЕ АПИ СУКААА os.loadAPI("System/API/Settings") -- подгружаем АПИ. да, выглядет как говно, обращение к АПИ на середине кода, но мне не привыкать. а что было в SpeedOS 1.3?
